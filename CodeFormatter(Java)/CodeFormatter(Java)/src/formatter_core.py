...
class FormatterCore:
    ...
    def _format_special(self, tokens):
        ...
        if (isinstance(self.cur, java_lexer.Identifier)
            and tokens[self.idx+1].value == ':' and self.stack[-1].value not in ('for', '?', 'assert')):
        ...
        if cur.value == '(':
            ...
            if stack[-1].value == '[]':
                ...
                if isinstance(pre, java_lexer.Keyword) and pre.value in self.b_keywords:
                    ...
                    if isinstance(stack[-2], java_lexer.Keyword) and stack[-2].value in self.p_keywords:
                        # whitespaces set in ')'
    ...
    def _format_operator(self, tokens):
        ...
        def _consume_template(self, idx):
            # template can include |<|,|>|?|extends|super|&|Identifier|BasicType
            ...
            while idx < len(tokens) and state > 0 and open_cnt > 0:
                ...
                if state == 1:
                    ...
                    elif isinstance(cur, java_lexer.BasicType): next_state = 5
                elif state == 2:
        ...
        if (cur.is_prefix() and (cur.value not in ('+', '-') or (
            ...
            if cur.is_assignment(): flag = space_around_assignment
                ...
                if cur.value == '<':
                    idx, res = _consume_template(self, self.idx + 1)
                    if res:
                        # new Type<...>
                        self.idx = idx - 1
